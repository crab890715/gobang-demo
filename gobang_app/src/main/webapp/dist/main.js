		...
		(
				{
							...
							function(require, module, exports) {
								...
								var Board = function(container, status) {
									...
									this.container.on("click", function(e) {
										if (self.lock || !self.started)
											return;
										var y = e.offsetX, x = e.offsetY;
										x = Math.floor((x + self.offset)
												/ self.step) - 1;
										y = Math.floor((y + self.offset)
												/ self.step) - 1;
									...
									this.worker.onmessage = function(e) {
										...
										self
												.setStatus("电脑下子("
														...
														+ e.data[1]
														+ "), 用时"
														+ ((new Date() - self.time) / 1000)
														+ "秒");
										var that = self;
										setLocation(e.data[0], e.data[1], function(x,y){that.set(x, y, 1);});
									}
								...
								Board.prototype.start = function() {
									...
									var that = this;
									setInit(7, 7, 15, 15, function(x,y){
										that.set(x, y, 1);
									});
								}
								...
								Board.prototype.draw = function() {
									...
									container.find(".chessman, .indicator")
											.remove();
										...
										for (var j = 0; j < board[i].length; j++) {
											if (board[i][j] != 0) {
												var chessman = $(
														...
														.appendTo(container);
												if (board[i][j] == 2)
													chessman.addClass("black");
												chessman.css("top", this.offset
														+ i * this.step);
												chessman.css("left",
														this.offset + j
																* this.step);
					...
					3 : [ function(require, module, exports) {
						/*
						 * 棋型表示 用一个6位数表示棋型，从高位到低位分别表示 连五，活四，眠四，活三，活二/眠三，活一/眠二,
						 * 眠一
						 */
